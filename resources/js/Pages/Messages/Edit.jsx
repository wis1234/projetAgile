...
export default function Edit({ message, projects, users }) {
    const { errors, flash } = usePage().props;
    const [values, setValues] = useState({
        content: message.content || '',
        project_id: message.project_id || projects[0]?.id || '',
        user_id: message.user_id || users[0]?.id || '',
    });
    ...
    const handleSubmit = e => {
        e.preventDefault();
        setSubmitting(true);
        Inertia.put(route('messages.update', message.id), values, {
            onSuccess: () => {
                setSubmitting(false);
    ...
    return (
        <div className="flex flex-col h-full w-full">
            <div className="max-w-xl w-full mx-auto">
                <h1 className="text-2xl font-bold mb-4">Éditer le message</h1>
