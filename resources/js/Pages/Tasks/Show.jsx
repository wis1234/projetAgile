  ...
  useEffect(() => {
    ...
    channel.listen('.comment.posted', (e) => {
      ...
      const insertRealtimeComment = (commentsList, inc) => {
        ...
        const existsRec = (list) => list.some(c =>
          String(c.id) === incId || (c.replies?.length && existsRec(c.replies))
        );
        if (existsRec(commentsList)) return commentsList;

        // Toujours ajouté en bas (racine), comme un message normal
        return [inc, ...commentsList];
      };
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-3 mb-5">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-4 shadow-sm flex flex-col justify-between">
            <div className="flex items-start gap-3">
              <Link
                href="/tasks"
                className="flex-shrink-0 bg-gray-100 hover:bg-blue-50 dark:bg-gray-700 dark:hover:bg-gray-600 text-gray-500 hover:text-blue-600 dark:text-blue-400 p-2.5 rounded-lg transition-colors duration-200"
              >
                <FaArrowLeft className="text-sm" />
              </Link>
              <div className="min-w-0">
                <h1 className="text-base md:text-lg font-semibold text-gray-800 dark:text-gray-100 tracking-tight leading-snug">
                  {task.title}
                </h1>
                {task.project && (
                  <Link
                    href={`/projects/${task.project.id}`}
                    className="inline-flex items-center gap-1.5 text-xs text-blue-600 hover:text-blue-800 dark:text-blue-400 mt-1"
                  >
                    <FaProjectDiagram className="text-[10px]" /> {task.project.name}
                  </Link>
                )}
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-4 shadow-sm flex flex-col gap-3">
            <div className="flex items-center justify-between">
              <h3 className="text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wide">
                Détails rapides
              </h3>
            </div>
              ...
              {task.assigned_user ? (
                <>
                  <img
                    ...
                    className="w-9 h-9 rounded-full object-cover border-2 border-blue-200 dark:border-blue-700"
                  />
                  <div className="min-w-0">
                    <p className="text-sm font-semibold text-gray-900 dark:text-white truncate leading-tight">{task.assigned_user.name}</p>
                    <p className="text-[11px] text-gray-500 dark:text-gray-400">Assigné à cette tâche</p>
                  </div>
                </>
              ) : (
                <>
                  <FaUserCircle className="w-9 h-9 text-gray-300 dark:text-gray-600" />
              ...
              {canEditTask && (
                <>
                  <Link
                    ...
                    className="bg-gradient-to-r from-blue-500 to-indigo-600 hover:from-blue-600 hover:to-indigo-700 text-white px-3.5 py-2 rounded-lg font-semibold flex items-center gap-1.5 transition-all duration-200 text-xs shadow-sm shadow-blue-500/20 hover:shadow-md hover:shadow-blue-500/30"
                  >
                  ...
                  <button
                    ...
                    className="bg-white dark:bg-gray-700 hover:bg-red-50 dark:hover:bg-red-900/30 text-red-600 dark:text-red-400 border border-red-200 dark:border-red-800 px-3.5 py-2 rounded-lg font-semibold flex items-center gap-1.5 transition-all duration-200 text-xs"
                  >
        ...
        >
          <FaEnvelope
            className={`w-3.5 h-3.5 flex-shrink-0 transition-colors duration-200 ${
              shareDiscussionEmail ? 'text-white animate-pulse' : 'text-blue-200'
            }`}
          />
          <input
            type="checkbox"
          ...
          />
          <span className={`relative w-8 h-4 rounded-full transition-colors duration-200 ${
            shareDiscussionEmail ? 'bg-orange-200' : 'bg-white/25'
          }`}>
            <span className={`absolute top-0.5 left-0.5 w-3 h-3 rounded-full shadow-sm transition-transform duration-200 ${
              shareDiscussionEmail ? 'bg-orange-600 translate-x-4' : 'bg-white'
            }`} />
          </span>
        </label>
      </div>
