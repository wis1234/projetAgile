...
import {
  ...
  FaTasks, FaUsers, FaChevronRight, FaDownload,
} from 'react-icons/fa';
...
const HeroSkeleton = () => (
  <section className="bg-gradient-to-b from-slate-950 to-slate-900 px-5 pb-7 pt-5 rounded-b-[28px]">
    <div className="flex items-center justify-between">
      <div className="space-y-2">
        <div className="h-2.5 w-20 rounded bg-white/10 animate-pulse" />
        <div className="h-5 w-40 rounded bg-white/10 animate-pulse" />
      </div>
      <div className="h-11 w-11 rounded-full bg-white/10 animate-pulse flex-shrink-0" />
    </div>
...
const StatCardSkeleton = () => (
  <div className="rounded-2xl border border-slate-100 bg-white p-4 shadow-sm">
    ...
    <Pulse className="mt-3.5 h-6 w-10" />
    <Pulse className="mt-2 h-3 w-16" />
  </div>
);

const SectionSkeleton = ({ lines = 3 }) => (
  <div className="bg-white rounded-3xl border border-slate-100 shadow-sm p-[18px]">
    ...
    <MobileLayout title="Accueil" subtitle={`Bonjour ${firstName}`} fullBleed>
      ...
      {error && (
        <div className="mx-4 mt-3 px-3 py-2 bg-red-50 border border-red-200 rounded-xl text-xs text-red-600 flex items-center justify-between gap-2">
          <span>{error}</span>
          <button onClick={fetchDashboard} className="font-semibold underline flex-shrink-0">Réessayer</button>
        </div>
      )}
